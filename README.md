...
| 취향 기반 추천 | `#/mypage` (음식/라이프/무드/동행 카드) → 자동 저장, Kakao 결과에 `applyAllPreferences()` 로 필터·랭킹 | (v0.4부터 `#/preferences`는 `#/mypage`로 리다이렉트) |
| 동행 빠른 토글 | 홈 상단 chip | 혼자/데이트/이성친구/친구/가족/회식 클릭 즉시 저장, 결과 정렬에 가중치 |
| 로그인 | `#/login` | Google / Kakao / Naver 3-way OAuth |
| 마이페이지 | `#/mypage` | 프로필·활동 통계·4개 취향 확장형 카드(즉시 저장)·저장 미리보기·로그아웃·탈퇴 |
...
│   │   └── share.js          # SITE_ORIGIN 상수 + sharePlace + mapLinks (카카오맵/네이버지도/구글맵 deep-link)
│   ├── views/
│   │   ├── HomeView.js       # 한옥 hero + 검색 + 동행 chip + 17개 시·도 grid + 마이페이지 CTA
...
│   │   ├── MyPageView.js     # 프로필·활동 통계·4개 확장형 취향 카드(음식/라이프/무드/동행, 즉시 저장)·저장 미리보기·계정 관리
...
VITE_NAVER_PROXY_URL=...     # Naver Local/Image API용 Cloudflare Worker 프록시 엔드포인트
```
...
리포 **Settings → Secrets and variables → Actions**에 다음 10개가 등록되어 있어야 합니다:
...
| `VITE_NAVER_PROXY_URL` | Naver Local/Image API용 Cloudflare Worker 프록시 URL (CORS 우회) |
...
- [SETUP.md](SETUP.md) — 3-way OAuth(Google/Kakao/Naver) 및 Naver Cloudflare Worker 프록시 세팅 가이드
- [SETUP-V2.md](SETUP-V2.md) — v1.0 운영 강화 가이드 (App Check, Firestore 보안 규칙, 검색엔진 등록, 커스텀 도메인, 번들 최적화, FCM 푸시)
